/** Shared labels/options for create + edit event forms */
...
/** Initial recurrence fields from API event document */
...
/** Reminder offsets as a Set for checkbox UI */
export function reminderSetFromEvent(event) {
  const arr =
    event?.reminderOffsetsMinutes?.length > 0 ? event.reminderOffsetsMinutes : [1440];
  return new Set(arr);
}
